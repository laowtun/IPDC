...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // ─── API: visitor fallback (Cloudflare request.cf) ───
...
<style>
:root{--bg:#0a0e17;--card:#111827;--border:#1e293b;--text:#e2e8f0;--muted:#94a3b8;--accent:#3b82f6;--accent2:#8b5cf6;--green:#22c55e;--red:#ef4444;}
...
.header h1{font-size:2em;background:linear-gradient(135deg,var(--accent),var(--accent2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:6px;}
.header p{color:var(--muted);font-size:1em;}
.search-box{display:flex;gap:10px;margin-bottom:28px;}
.search-box input{flex:1;padding:13px 18px;border-radius:10px;border:1px solid var(--border);background:var(--card);color:var(--text);font-size:1em;outline:none;}
.search-box input:focus{border-color:var(--accent);}
.search-box input::placeholder{color:var(--muted);}
.search-box button{padding:13px 24px;border-radius:10px;border:none;background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;font-size:1em;font-weight:600;cursor:pointer;}
...
.info-item{padding:10px 14px;background:rgba(255,255,255,.03);border-radius:8px;border:1px solid rgba(255,255,255,.05);}
...
#globe-container{width:100%;height:380px;border-radius:10px;overflow:hidden;background:#000;}
...
@media(max-width:600px){.search-box{flex-direction:column;}.header h1{font-size:1.5em;}.ip-display{font-size:1.2em;}.info-grid{grid-template-columns:1fr;}#globe-container{height:260px;}.dual-row{flex-direction:column;}}
</style>
</head>
<body>
<div class="container">
...
// ─── Cloudflare Radar: 同时获取 IPv4 + IPv6 ───
...
// ─── Auto-detect visitor (dual-stack) ───
